...
#'
#' @description
#' \code{saveArgosData} saves an object of type argosData to folder.
#'
...
#'
#' @export
saveArgosData <- function(argosData, file, envir=NULL, overwrite=F) {
    if (missing(argosData)){
        stop("Must specify argosData")
    }
    if (missing(file)){
        stop("Must specify file")
    }
    if (!inherits(x = argosData, what =  c("argosData"))){
        stop("Data not of class argosData")
    }
    ...
    if(is.null(argosData$metaData$call$sampleSize)){  # fixed a bug when sampleSize is NULL
        argosData$metaData$call$sampleSize <- 'NULL'
    }
    
    Andromeda::saveAndromeda(argosData$covariateData, file = file.path(file, "covariates"), maintainConnection = T)
    saveRDS(argosData$timeRef, file = file.path(file, "timeRef.rds"))
    saveRDS(argosData$cohorts, file = file.path(file, "cohorts.rds"))
    saveRDS(argosData$outcomes, file = file.path(file, "outcomes.rds"))
    saveRDS(argosData$metaData, file = file.path(file, "metaData.rds"))
}

#' Load the cohort data from a folder
#'
#' @description
#' \code{loadArgosData} loads an object of type argosData from a folder in the file
#' system.
#'
...
#'
#' @return
#' An object of class argosData.
#'
#' @examples
#' # todo
#'
#' @export
loadArgosData <- function(file, readOnly = TRUE) {
    if (!file.exists(file))
