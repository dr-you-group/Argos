...
#' createRestrictArgosDataSettings define extra restriction settings when calling getArgosData
#'
#' @description
#' This function creates the settings used to restrict the target cohort when calling getArgosData
#' @details
...
#' 
#' @return
#' A setting object of class \code{restrictArgosDataSettings} containing a list getArgosData extra settings
#'
#' @export
createRestrictArgosDataSettings <- function(
        studyStartDate = "",
        studyEndDate = "",
        firstExposureOnly = F,
        washoutPeriod = 0,
    ...
    class(result) <- 'restrictArgosDataSettings'
    return(result)
}
...
#' @param cdmDatabaseId                  A string with a unique identifier for the database and version - this is stored in the argos object for future reference and used by the shiny app (defaults to cdmDatabaseSchema if not specified)
...
#' 
#' @return
#' A list with the the database specific settings (this is used by the runMultipleArgos function and the skeleton packages)
#'
#' @export
...
#' @param restrictArgosDataSettings  Extra settings to apply to the target population while extracting data.  Created using \code{createRestrictArgosDataSettings()}.                                    
#'
#' @return
#' Returns an object of type \code{argosData}, containing information on the cohorts, their
...
#'
#' @export
getArgosData <- function(connectionDetails,
                         cdmDatabaseSchema,
                         oracleTempSchema = cdmDatabaseSchema,
                         targetId,
                         outcomeIds,
                         restrictArgosDataSettings,
                         cohortDatabaseSchema = cdmDatabaseSchema,
                         cohortTable = "cohort",
                         outcomeDatabaseSchema = cdmDatabaseSchema,
                         outcomeTable = "cohort",
    ...
    checkIsClass(restrictArgosDataSettings, 'restrictArgosDataSettings')
    if(is.null(targetId))
    ...
    if(!is.null(restrictArgosDataSettings$sampleSize))  writeLines(paste("\n Sampling ",restrictArgosDataSettings$sampleSize, " people"))
    ...
    renderedSql <- SqlRender::loadRenderTranslateSql("CreateCohorts.sql",
                                                     ...
                                                     target_id = targetId,
                                                     study_start_date = restrictArgosDataSettings$studyStartDate,
                                                     study_end_date = restrictArgosDataSettings$studyEndDate,
                                                     first_only = restrictArgosDataSettings$firstExposureOnly,
                                                     washout_period = restrictArgosDataSettings$washoutPeriod,
                                                     use_sample = !is.null(restrictArgosDataSettings$sampleSize),
                                                     sample_number = restrictArgosDataSettings$sampleSize
    )
    ...
    metaData.cohort <- list(targetId = targetId)
    ...
    #Need: covariate data for only outcomes, with sampling
    ...
    # add indexes for tidyCov + covariate summary (is not available for current Andromeda v0.3.2)
    ...
    # metaData$databaseDetails <- connectionDetails
    # metaData$databaseDetails$connectionDetails = NULL
    # metaData$databaseDetails$connection = NULL
    metaData$restrictArgosDataSettings <- restrictArgosDataSettings
    ...
    class(result) <- "argosData"
    return(result)
}


#' @export
print.argosData <- function(x, ...) {
    writeLines("argosData object")
    writeLines("")
...
#' @method summary argosData
#' @export
summary.argosData <- function(object,...){
    ...
    class(result) <- "summary.argosData"
    return(result)
}

#' @export
print.summary.argosData <- function(x, ...) {
    writeLines("argosData object summary")
    writeLines("")
