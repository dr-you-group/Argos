...
#'                               specified).
#' @param startAnchor	           The anchor point for the start of the risk window. Can be "cohort start", "cohort end", or "exact date".
...
#'                               specified).
#' @param endAnchor              The anchor point for the end of the risk window. Can be "cohort start", "cohort end", or "exact date".
...
createStudyPopulationSettings <- function(
    ...
    result <- list(
        ...
        changeCohortStartDate = changeCohortStartDate,
        newCohortStartDate = newCohortStartDate
    )
...
#'
#' @param argosData      An object of type \code{argosData} as generated using
#'                              \code{getargosData}.
...
#'                              cohorts in the \code{argosData} object.
#'
#' @return
...
createStudyPopulation <- function(
    ...
    changeCohortStartDate <- populationSettings$changeCohortStartDate
    newCohortStartDate <- populationSettings$newCohortStartDate
    
    # parameter checks
    if(!inherits(x = argosData, what = c('argosData'))){
        ParallelLogger::logError('Check argosData format')
        stop('Wrong argosData input')
    }
    ...
    if (is.null(population)) {
        population <- argosData$cohorts
    }
    
    # save the metadata (should have the ?targetId, outcomeId, argosDataSettings and population settings)
    metaData <- attr(population, "metaData")
    metaData$restrictArgosDataSettings <- argosData$metaData$restrictArgosDataSettings
    ...
    if(is.null(metaData$attrition)){
        metaData$attrition <- attr(argosData$cohorts,  'metaData')$attrition
    }
    if(!is.null(metaData$attrition)){
    ...
    # Change cohort start date
    if (changeCohortStartDate){
        population <- population %>%
            mutate(newCohortStartDate = as.Date(newCohortStartDate, "%Y%m%d"))
    } else {
        population <- population %>%
            mutate(newCohortStartDate = .data$cohortStartDate)
    }
    
    population <- population %>%
        mutate(changeCohortStartDate = changeCohortStartDate) %>%
        # mutate(newCohortStartDate = ifelse(.data$changeCohortStartDate, as.Date(newCohortStartDate, "%Y%m%d"), .data$cohortStartDate))%>%
        mutate(startDateDiff = as.numeric(.data$newCohortStartDate - .data$cohortStartDate)) %>%
        filter(.data$startDateDiff >= 0) %>%
        mutate(cohortStartDate = .data$newCohortStartDate) %>%
        mutate(daysFromObsStart = as.numeric(.data$cohortStartDate - .data$observationPeriodStartDate)) %>%
        mutate(daysToCohortEnd = .data$daysToCohortEnd - .data$startDateDiff) %>%
        mutate(daysToObsEnd = as.numeric(.data$observationPeriodEndDate - .data$cohortStartDate))
        
    
    # ADD TAR
    oId <- outcomeId
    population <- population %>% 
        dplyr::mutate(startAnchor = startAnchor, endAnchor = endAnchor) %>%
        dplyr::mutate(startDay = ifelse(.data$startAnchor == 'exact date', as.Date(riskWindowStart, "%Y%m%d")-.data$cohortStartDate,riskWindowStart)) %>%
        dplyr::mutate(endDay = ifelse(.data$endAnchor == 'exact date', as.Date(riskWindowEnd, "%Y%m%d")-.data$cohortStartDate,riskWindowEnd)) %>%
        dplyr::mutate(tarStart = ifelse(.data$startAnchor == 'cohort end', .data$startDay+ .data$daysToCohortEnd, .data$startDay),
                      tarEnd = ifelse(.data$endAnchor == 'cohort end', .data$endDay+ .data$daysToCohortEnd, .data$endDay)) %>%
    ...
    outcomeTAR <- population %>% 
        dplyr::inner_join(argosData$outcomes, by ='rowId') %>% 
        dplyr::filter(.data$outcomeId == get('oId'))  %>% 
        dplyr::mutate(daysToEvent = .data$daysToEvent - .data$startDateDiff) %>%
    ...
    attrRow <- population %>% dplyr::group_by() %>%
        dplyr::summarise(outcomeId = get('oId'),
                         description = 'Initial argosData cohort or population',
